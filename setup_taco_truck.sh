...
if ! test -f $UDEV_FILE; then
    ...
    echo "Udev rule for backlight already present."
fi

# Service enable
if ! test -f /etc/systemd/system/taco_truck.service; then
    echo "Enabling taco_truck systemd service..."
    sudo ln -s /home/dan/git/taco_hu/taco_truck.service /etc/systemd/system/
    sudo systemctl enable taco_truck.service
else
    echo "taco_truck systemd service already enabled."
fi